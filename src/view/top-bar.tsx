...
export default function topBar(state: AppState): VNode {
    ...
    const topBarImageSectionClass = style(csstips.height(55), csstips.flex1, csstips.centerCenter, {
        fontFamily:    'Bangers',
        fontSize:      '2em',
        color:         '#000',
        letterSpacing: '1px'
    ...
    return (
        <nav className={topBarClass}>
            <div className={topBarImageSectionClass}>
                <img src={logo} className={imageSizeClass}/>
            </div>
            <div className={topBarMenuSectionClass}>
                {Object.keys(AppData).map((section) =>
                                              <a className={linelessLinkClass}
                                                 href={`#${titleToAnchor(section)}`}>{section}</a>
                )}
            </div>
        </nav>
    )
